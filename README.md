...
`--base-url URL` overrides the detected endpoint; `--api-key KEY` overrides the env-var fallback. Inside telia, `/keys` lists every provider and marks which env vars are set so you can see at a glance what's available. The `/model` drop-down pre-populates with the well-known models per provider — Claude (`opus-4-7`, `sonnet-4-6`, `haiku-4-5-20251001`), OpenAI (`gpt-5`, `gpt-5-mini`, `o3`, `o4-mini`), Google (`gemini-2.5-pro`, `gemini-2.5-flash`), xAI (`grok-4`, `grok-3`), DeepSeek (`deepseek-chat`, `deepseek-reasoner`), Mistral (`mistral-large-latest`, `codestral-latest`), Groq (`groq:llama-3.3-70b-versatile`, `groq:deepseek-r1-distill-llama-70b`), OpenRouter (`openrouter:anthropic/claude-opus-4-7`, `openrouter:openai/gpt-5`) — alongside whatever Ollama has cached locally, so switching is one Tab away.
...
- **Slash commands** — `/reset`, `/clear`, `/save NAME`, `/load NAME`, `/delete NAME`, `/list`, `/model [NAME]`, `/keys`, `/theme [NAME]`, `/notify [on|off]`, `/show`, `/help`, `/quit`. Vim users get the same set as `:q`, `:w NAME`, `:e NAME`, `:colo NAME`, etc.

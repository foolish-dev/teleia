...
- **Slash commands** — `/reset`, `/clear`, `/save NAME`, `/load NAME`, `/delete NAME`, `/list`, `/model [NAME]`, `/keys`, `/plan`, `/build`, `/auto`, `/prompt [NAME]`, `/log [ARGS]`, `/theme [NAME]`, `/notify [on|off]`, `/show`, `/help`, `/quit`. Vim users get the same set as `:q`, `:w NAME`, `:e NAME`, `:colo NAME`, etc.
- **`/log` → gitlogue** — suspends the telia TUI and hands control to [`gitlogue`](https://github.com/unhappychoice/gitlogue), a cinematic git-commit replay tool by [@unhappychoice](https://github.com/unhappychoice). When it exits, telia restores the alt screen and mouse capture. Pass arguments through: `/log -c HEAD~5..HEAD`, `/log diff`, etc. Install with `cargo install gitlogue` if it isn't on `PATH` — telia surfaces a one-line hint otherwise.
